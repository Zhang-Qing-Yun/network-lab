...
/**
 * @description： 基于UDP的SR协议
 * @author: 張青云
 * @create: 2021-10-31 21:56
 **/
public class SR implements ReliableProtocol {
    /*
    * 相较于GBN需要做两点：发送端给已发送的每个报文段都设置一个定时器，超时只重传这一个报文段；TODO
    *                   接收端缓存接收到的每个报文段，然后在有序时再传给上层并移动窗口。
    *
    * 注意：窗口长度要小于等于序号空间的一半
    * */
