...
/**
 * @description： 基于UDP的滑动窗口协议（阻塞式）
 * @author: 張青云
 * @create: 2021-10-31 16:07
 **/
public class GBN implements ReliableProtocol {
    //  目的主机地址
    ...
    //  最大尝试接收次数，超出这个值则认为数据接收完整了
    private int receiveMaxTime = 5;
    ...
    public void send(byte[] content) throws Exception {
        ...
        do {
            ...
            while(timers.size() < windowSize && sendIndex < content.length && sendSeq < 256) {
                ...
                sendIndex += length;
                System.out.println("发送数据包：seq " + sendSeq);
                sendSeq ++;
            }
            ...
            try {
                while(!checkWindow(timers)) {
                    ...
                    // 取出序列号并计算当前是滑动窗口中的第几个
                    int index = (int)((recv[0] & 0x0FF) - base);
                    // GBN是累计确认，所以将index之前的都确认了
                ...
                for(int i = 0; i < timers.size(); i ++) {
                    ...
                    if(tempTime != -1) {
                        ...
                        System.err.println("重新发送数据包：seq " + (i + base));
                        timers.set(i, 0);
                    }
                }
            }
